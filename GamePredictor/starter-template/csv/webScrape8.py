...
filename = "fourdata2.csv"
f = open(filename, "w")
headers = "category,val0,val1\n"
f.write(headers)
for eachItem in ppg:
	if(eachItem.find('a') is not None and eachItem.find('a').getText() == "New York"):
	...
	if(eachItem.find('a') is not None and eachItem.find('a').getText() == "Toronto"):
		allStats = eachItem.find_all('td')
		x=1
