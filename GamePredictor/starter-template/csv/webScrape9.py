...
filename = "fourdata3.csv"
f = open(filename, "w")
headers = "category,val0,val1\n"
f.write(headers)
for eachItem in ppg:
	if(eachItem.find('a') is not None and eachItem.find('a').getText() == "Boston"):
	...
	if(eachItem.find('a') is not None and eachItem.find('a').getText() == "Washington"):
		allStats = eachItem.find_all('td')
		x=1
